...
export class Tunnel01 {
    ...
    private _centerBezierPoints: Vector3[];




    constructor( theCanvasElementDomId: string ) {
    ...
    createScene(): void {
        ...
        this._camera = new ArcRotateCamera("cam", -Math.PI / 2, Math.PI / 2, CAMERA_RADIUS, Vector3.Zero(), this._scene);
        ...
        this.mazeBorderNumSteps_calcAndSet(  OUTERRADIUS, MAXSTEPLEN, BORDER_ANGLE_START, BORDER_ANGLE_STOP);
        ...
        this.mazeSheetNumStruts_calcAndSet( OUTERLENGTH, MAXSTRUTLEN);

        this._outerBorderMesh = this.mazeOuterBorder( OUTERRADIUS);
        this._outerSheetMesh  = this.mazeOuterSheet( OUTERRADIUS, OUTERLENGTH);

        this._innerBorderMesh = this.mazeInnerBorder( INNERRADIUS);
        this._innerSheetMesh  = this.mazeInnerSheet( INNERRADIUS, INNERLENGTH);
    ...
    centerBezier( theRadius: double): Mesh {
        const someBorderPoints = this.centerBezierPoints( theRadius);
        return MeshBuilder.CreateLines( "centerBezier01", { points: someBorderPoints});
    }


    centerBezierPoints( theRadius: double): Vector3[] {

        if( !( this._centerBezierPoints == null)) {
            return this._centerBezierPoints;
        }

        this._outerRadius = theRadius;

        this._centerBezierPoints = this.centerBezierPoints_calc( theRadius);
        return this._centerBezierPoints;
    }



    centerBezierPoints_calc( theRadius: double): Vector3[] {
        /*
        ...
        for( let anStepIdx=0; anStepIdx <= this._borderNumSteps; anStepIdx++) {
            ...
            const aBorderPoint = this.centerBezierPoint( aCenter, theRadius, anAngle);
    ...
    mazeBorderNumSteps_calcAndSet( theRadius: double, theMaxStepLen: double, theAngleStart: double, theAngleStop: double): int {
    ...
    mazeSheetNumStruts_calcAndSet( theLength: double, theMaxStrutLen: double): int {
    ...
    mazeOuterBorder( theRadius: double): Mesh {
        const someBorderPoints = this.mazeOuterBorderPoints( theRadius);
    ...
    mazeInnerBorder( theRadius: double): Mesh {
        const someBorderPoints = this.mazeInnerBorderPoints( theRadius);
    ...
    mazeOuterSheet( theRadius: double, theLength: double): Mesh {

        const aVertexData = this.mazeOuterSheetVertexData( theRadius, theLength);
    ...
    mazeInnerSheet( theRadius: double, theLength: double): Mesh {

        const aVertexData = this.mazeInnerSheetVertexData( theRadius, theLength);
